...
void oled_init() {
    ...
    i2c_send_byte_array(oled, OLED128X64_ADDR, init_commands, sizeof(init_commands)/sizeof(uint8_t));
}

void oled_turn_on_all_pixels(){
    i2c_send_word(oled, OLED128X64_ADDR, SSD1306_COMMAND, SSD1306_DISPLAY_ON);
    i2c_send_word(oled, OLED128X64_ADDR, SSD1306_COMMAND, SSD1306_ENTIRE_DISPLAY_ON);
}

void oled_show_RAM_content(){
    i2c_send_word(oled, OLED128X64_ADDR, SSD1306_COMMAND, SSD1306_DISPLAY_ON);
    i2c_send_word(oled, OLED128X64_ADDR, SSD1306_COMMAND, SSD1306_ENTIRE_DISPLAY_RESUME);
}
...
void oled_off(){
    i2c_send_word(oled, OLED128X64_ADDR, SSD1306_COMMAND, SSD1306_DISPLAY_OFF);
}
...
void oled_flush(){
    ...
    i2c_send_byte_array_w_ctl_byte(oled, OLED128X64_ADDR, SSD1306_COMMAND, cmds, sizeof(cmds));
    i2c_send_byte_array_w_ctl_byte(oled, OLED128X64_ADDR, SSD1306_DATA, oled_canvas, sizeof(oled_canvas));
}
...
void oled_flush_area(uint8_t seg_start, uint8_t seg_end, uint8_t col_start, uint8_t col_end) {
    ...
    //     };
    //     i2c_send_byte_array_w_ctl_byte(oled, OLED128X64_ADDR, SSD1306_COMMAND, cmd_page, 3);

    //     uint16_t offset = page * 128 + col_start;
    //     uint16_t len = col_end - col_start + 1;

    //     i2c_send_byte_array_w_ctl_byte(oled, OLED128X64_ADDR, SSD1306_DATA, &oled_canvas[offset], len);
    // }
}
